...
#
# v.30
# Added error reporting to errors.txt
#
# v. 28
# Reduced GPS Coordinate Preciness to 0.1 from 0.01
# Cleanup
...
function Bootstrap(){
    ...
    Print_In_Color "*** Script Version              : $SCRIPTVERSION"
...
if [ -d "$CURRDIR" ]; then
    Print_In_Color "*** Working Directory           : $CURRDIR" 
fi


# Find Where The Applications Are
# Used readlink to get absolute path
APPDIR=$(readlink -f "${CURRDIR}/../Documents/Applications/Windows/")

if [ -d "$APPDIR" ]; then
    Print_In_Color "*** Application Directory Found : $APPDIR" 
fi
...
SOURCEDIR="${CURRDIR}/../_Sandbox"
#SOURCEDIR="${CURRDIR}/../Pictures/Unknown"
#SOURCEDIR="${CURRDIR}/../Pictures/2012"
#SOURCEDIR="${CURRDIR}/Source"
    Print_In_Color "*** Source Directory            : $SOURCEDIR" 
    
TARGETDIR="${CURRDIR}/Target"
    Print_In_Color "*** Target Directory            : $TARGETDIR"
...
TOTALFILECOUNT=$(find $SOURCEDIR -type f -not -name '*.sh' -not -name '.picasa.ini' | wc -l)
    Print_In_Color "*** File Count in Source Dir    : $TOTALFILECOUNT"
...
function Get_FullDate_From_Picture {
	...
	#If there is no date info
	#if [ "$tmpdate" == "" ] || [ "${tmpdate:0:4}" == "0000" ]; then
	if [ "$tmpdate" == "" ] || [[ "$tmpdate" == *"0000"* ]]; then
